...
<body x-data="" class="is-header-blur" x-bind="$store.global.documentBody">
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            // Fetch the call types from the call_types table to check for "Prank Call"
            $call_types_query = "SELECT id, call_type, service_type_id FROM call_types WHERE call_type = 'Prank Call'";
...
<div style="border: 5px solid crimson;" class="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-5 lg:grid-cols-2">
    ...
    <?php endforeach; ?>
        ...
        <script>
                        ...
                        <form class="grid grid-cols-2 gap-8 mt-6">
                            <div class="col-span-2 space-y-6">
                                <div class="grid grid-cols-3 gap-6">
                                    <input type="hidden" name="service_type" value="${id}" readonly disable>
                                    <div>
                                ...
                                <div>
                                    ...
                                    <textarea id="remarks" name="remarks" value="${remarks}" rows="4" class="form-textarea w-full resize-none rounded-lg bg-slate-150 p-2.5 placeholder:text-slate-400 dark:bg-navy-900 dark:placeholder:text-navy-300" readonly></textarea>
                                </div>
                            </div>
                        </form>
                    `;
            ...
            // Simulate deleting the case
            function deleteCase(id) {
                // Delete logic goes here
                alert("Case with ID " + id + " deleted");
                closeModal();
            }
